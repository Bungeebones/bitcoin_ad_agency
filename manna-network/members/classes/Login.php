...
class Login
{
 ...
 // user try to change his wdgts_lnk_num
            } elseif (isset($_POST["user_edit_submit_wdgts_lnk_num"])) {

                // function below uses use $_SESSION['user_id'] et $_SESSION['user_email']
                $this->editUserWdgts_Lnk_Num($_POST['wdgts_lnk_num']);
...
if (array_key_exists('user_id', $_SESSION)) {
    ...
    /**
     * Edit the user's wdgts_lnk_num, provided in the editing form
     */
    public function editUserWdgts_Lnk_Num($wdgts_lnk_num)
    {
        // prevent database flooding
        $user_email = substr(trim($user_email), 0, 64);
         ...
         if ($this->databaseConnection()) {
            ...
            $result_row = $query_user->fetchObject();
                ...
                $query_edit_wdgts_lnk_num->execute();

                if ($query_edit_wdgts_lnk_num->rowCount()) {
                    $_SESSION['wdgts_lnk_num'] = $wdgts_lnk_num;
                    $this->messages[] = MESSAGE_EMAIL_CHANGED_SUCCESSFULLY . $wdgts_lnk_num;
                } else {
                    $this->errors[] = MESSAGE_EMAIL_CHANGE_FAILED;
                }
