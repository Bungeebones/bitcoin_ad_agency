     ...
     echo "User Has submitted the form and entered this customer db name : <b> ".$dbnamecustomers ."</b>";
      echo "<br>User Has submitted the form and entered this agent db name : <b>". $dbnameagents." </b>";
       ...
       $mysqli = new mysqli($servername, $username, $password);
        ...
        echo "<br>Connected to database sucessfully - configuration in manna-configs/db_cfg/agent_configs.php confirmed";
$constants_save_location = dirname( __FILE__, 4 )."/manna-configs/db_cfg/auth_constants.php";
//echo '<br>line 42 $constants_save_location is just one of many files we could try to access to see if the group is owned by www-data
$handle = fopen($constants_save_location, 'w') or die('<br> &nbsp;<h3>Cannot open the files downloaded/installed by Github. This is probably caused by them not being assigned to the proper group. They need to be in the www-data group and NOT in the group of the user that installed or downloaded or cloaned them from Github! To make those files and folders be in the group www-data then (from the command line [i.e. terminal]) do the following: <ol><li> cd to the folder one level above the location where manna network is installed (note if you cloned Manna Network it would the level where bitcoin_ad_agency is)</li><li>type this into the command line: chgrp -R www-data bitcoin_ad_agency (Note, if you renamed the "bitcoin_ad_agency" folder or if you created a diffently named folder and installed Manna Network there then change the name "bitcoin_ad_agency"  in the command to the folder name where you installed them. </li><li>You can check the ownership with the command ls -al which should give an output something like  
drwxrwxr-x 8 robert www-data   4096 Dec 15 14:34 bitcoin_ad_agency</li></ol></h3>
